...
# find the mean lat and lon in the selected df
mean_lat = df["Latitude"].mean()
mean_lon = df["Longitude"].mean()
...
app.layout = html.Div(
    [
        ...
        dcc.Dropdown(
            id="emotion-dropdown",
            options=[{"label": emotion, "value": emotion} for emotion in emotion_order],
            value="surprise",  # Set the default emotion
            style={"width": "30%"},  # Adjust the width of the dropdown
        ),
    ],
...
@app.callback(
    Output("emotion-density-map", "figure"), [Input("emotion-dropdown", "value")]
)
def update_emotion_heatmap(selected_emotion):
    # Filter the DataFrame based on the selected emotion
    selected_df = df[df["label"] == selected_emotion]
    min_density = selected_df["Density"].min()
    # calculate the mean while excluding 0s
    max_density = selected_df[selected_df["Density"] > 5]["Density"].mean() + 10
    ...
    fig = px.density_mapbox(
        ...
        center=dict(lat=mean_lat, lon=mean_lon),  # Center of the United States
        zoom=3,  # Adjust the zoom level as needed
        mapbox_style="stamen-terrain",  # You can choose a different map style
        title=f"Heatmap of {selected_emotion} Emotion",
        color_continuous_scale="Viridis",  # Adjust the color scale
        range_color=[min_density, max_density],  # Set the color scale range
        hover_data={
            "Latitude": False,
            "Longitude": False,
            "Density": True,
