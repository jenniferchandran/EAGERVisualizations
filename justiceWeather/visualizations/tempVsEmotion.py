...
# Drop rows where "Hiker Journal Link" is not in the weather data
emotion_percentages = emotion_percentages[
    emotion_percentages["Hiker Journal Link"].isin(weather_data)
]


# Add a new column "temp" to the DataFrame and append temperature information
def get_temp(link):
    if link in weather_data:
        first_entry = weather_data[link]["data"][0]
        return first_entry.get("temp", None)
    return None


emotion_percentages["temp"] = emotion_percentages["Hiker Journal Link"].apply(get_temp)

# Filter out rows where "temp" is None (if not all links have temperature information)
emotion_percentages = emotion_percentages.dropna(subset=["temp"])

# Merge emotion_percentages with the original DataFrame df
merged_df = pd.merge(
    df,
    emotion_percentages,
    on=["Hiker Journal Link", "year", "month", "Latitude", "Longitude"],
)
