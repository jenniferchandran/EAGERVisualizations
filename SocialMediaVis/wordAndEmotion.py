...
df = pd.read_csv('friday_meeting.csv', low_memory=False)
...
# Create a new column for each keyword with the count of occurrences, each occurance is 1 for the purpose of the data
for keyword, variations in keywords.items():
    df[keyword + '_Count'] = df.apply(count_keyword_occurrences, axis=1, args=(variations,))

# Keep only the required columns, reduced to year, state, and emotion_label 
columns_to_keep = ['year', 'State', 'emotion_label'] + [k + '_Count' for k in keywords]
df = df[columns_to_keep]

# Save the updated DataFrame to a new CSV file
df.to_csv('fri_analyzed.csv', index=False) 